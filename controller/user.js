...
const { secret, frontEnd } = require("../config/var");

// get User from id
exports.getAllUser = async (req, res) => {
	try {
		let data = await User.find({ _id: { $ne: req.auth._id } });
		let friends = await User.findOne({ _id: req.auth._id });
		friends = friends.contacts;
		friends = friends.map((cur) => cur.toString());
		data = data.filter((cur) => !friends.includes(cur._id.toString()));
		return res.json({ data });
	} catch (err) {
		return res.json({ err: "failed to get all user" });
	}
};
exports.getUserById = async (req, res, next, id) => {
	try {
		...
		return res
			.status(400)
			.json({ error: "No Such user exist form give Id" });
	}
};
...
exports.forgetPassword = async (req, res) => {
	try {
		...
		const data = await sendMail(
			email,
			`${frontEnd}/user/password-reset?token=${token}`,
			user.name,
		);
...
exports.createContact = async (req, res, next) => {
	try {
		...
		if (alreadyInList)
			return res.json({ error: "User Already exist in Contact list" });

		const updateduser = await User.findOneAndUpdate(
			{ _id: req.auth._id },
			{ $push: { contacts: newuser } },
		);
...
exports.removeContactPersonFromContacList = async (req, res, next) => {
	try {
		...
		contactList.map((item) => {
			if (item._id.toString() != userToRemove._id)
		...
		return res
			.status(400)
			.json({ error: "Failed to remove person from list" });
	}
};
// is isAuthenticated
